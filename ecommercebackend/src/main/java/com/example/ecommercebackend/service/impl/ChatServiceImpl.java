...
public class ChatServiceImpl implements ChatService {
    ...
    public ChatResponse askQuestion(ChatRequest request) {
        ...
        try {
            ...
            data = queryExecutionService.executeReadOnlyQuery(sqlQuery);
            System.out.println("DEBUG (Java): Data fetched, row count: " + data.size());
        } catch (Exception e) {
            ...
            try {
                ...
                if (fixResponse != null && fixResponse.get("sql_query") != null) {
                    ...
                    return createFallbackResponse("Database execution failed and AI Error Agent was unable to fix the SQL syntax.");
                }
            } catch (Exception ex) {
                System.out.println("DEBUG (Java): /fix_sql failed: " + ex.getMessage());
                return createFallbackResponse("Database execution failed again after AI retry attempt: " + ex.getMessage());
            }
        }
